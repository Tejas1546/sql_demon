...
export class PostgreSqlDriver implements IDatabaseDriver {
    ...
    async connect(): Promise<void> {
        ...
        this.client = typeof this.connectionConfig === "string"
    ...
    async execute(query: string, params?: unknown[]): Promise<DatabaseDriverResult> {
        ...
        const insertedId = typeof rowId === "number"
            ? rowId
            : typeof rowId === "string" && rowId.trim() !== "" && !Number.isNaN(Number(rowId))
                ? Number(rowId)
                : undefined;
        return {
    ...
    getUpsertQuery(tableName: string, columns: string[], conflictColumns: string[]): string {
        const placeholders = columns.map((_, index) => this.getNumberedPlaceholder(index + 1)).join(", ");
    ...
    getUpdateQuery(tableName: string, columns: string[], conditions: Record<string, unknown>): string {
        const setClause = columns.map((column, index) => `${column} = ${this.getNumberedPlaceholder(index + 1)}`).join(", ");
